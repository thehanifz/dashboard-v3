...
export default function PTLFilterManager({ records, statusColumnName = "Status Pekerjaan", onClose }: Props) {
  const { ptlActiveFilters, togglePtlFilter, clearPtlFilters } = useAppearanceStore();
      ...
      <div className="fixed top-0 right-0 h-full w-80 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-200"
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          ...
          {availableKeys.map((key) => {
            ...
            const selectedCount = ptlActiveFilters[key]?.length || 0;
              ...
              <div key={key} className="rounded-lg overflow-hidden"
                ...
                {isExpanded && (
                  <div className="p-3 max-h-60 overflow-y-auto space-y-1 custom-scrollbar"
                    ...
                    {values.map((val) => {
                      const isChecked = !!ptlActiveFilters[key]?.includes(val);
                      return (
                        <label key={val}
                          ...
                          <input type="checkbox"
                            ...
                            checked={isChecked}
                            onChange={() => togglePtlFilter(key, val)} />
        ...
        <div className="p-4 shrink-0 flex justify-between items-center"
          ...
          <button onClick={clearPtlFilters}
